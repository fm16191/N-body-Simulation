...
// 
void print_debug(particle_t* p, u64 n, char* DEBUG_filename, u64 step) {
  FILE* fp;
...
int main(int argc, char** argv) {
  ...
  fprintf(stderr, "N-body 3D Problem version 1.0 - Baseline\n\n");
  if (CONFIG) {
  ...
  for (u64 i = 0; i < steps; i++) {
    ...
    //GFLOPS
    const f32 h2 = (17 * h1 + 9.0 * (f32)n) * 1e-9;
