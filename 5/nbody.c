...
#define NB_VALUES_PACKED 8

// Horizontal addition of __m256
static inline f32 _hadd_ps(const __m256 a) {
...
void malloc_particle_t_SOA(particle_t_SOA* p, u64 n){
  p->x = aligned_alloc(sizeof(f32)*2*NB_VALUES_PACKED, sizeof(f32) * n);
  p->y = aligned_alloc(sizeof(f32)*2*NB_VALUES_PACKED, sizeof(f32) * n);
  p->z = aligned_alloc(sizeof(f32)*2*NB_VALUES_PACKED, sizeof(f32) * n);
  p->vx = aligned_alloc(sizeof(f32)*2*NB_VALUES_PACKED, sizeof(f32) * n);
  p->vy = aligned_alloc(sizeof(f32)*2*NB_VALUES_PACKED, sizeof(f32) * n);
  p->vz = aligned_alloc(sizeof(f32)*2*NB_VALUES_PACKED, sizeof(f32) * n);
}
...
void move_particles_SOA(particle_t_SOA* p, const f32 dt, u64 n) {
  ...
  __m256 d_2, soften, vdt;

  soften = _mm256_set1_ps(softening);
  ...
  for (u64 i = 0; i < n; i ++) {
    ...
    pxi = _mm256_set1_ps(p->x[i]);
    pyi = _mm256_set1_ps(p->y[i]);
    pzi = _mm256_set1_ps(p->z[i]);

    for (u64 j = 0; j < n; j += NB_VALUES_PACKED) {
      //Newton's law

      //// dx, dy, dz calculation. 3 Floating operations (3 loop total)
      dx = _mm256_sub_ps(_mm256_loadu_ps(p->x+j), pxi);
      dy = _mm256_sub_ps(_mm256_loadu_ps(p->y+j), pyi);
      dz = _mm256_sub_ps(_mm256_loadu_ps(p->z+j), pzi);

      //// d_2 Calculation : 6 Floating operations (9 loop total)
      d_2 = _mm256_mul_ps(dx, dx) + _mm256_mul_ps(dy, dy) + _mm256_mul_ps(dz, dz) + soften;

      //// d3_inv_sqrt_d_2 calculation. 5 Floating operations (14 loop total)
      d_2 = _mm256_rsqrt_ps(d_2);
      d_2 = _mm256_mul_ps(_mm256_mul_ps(d_2, d_2), d_2);

      //// Net force calculation. 6 Floating operations (20 loop total)
      fx = _mm256_fmadd_ps(dx, d_2, fx);
      fy = _mm256_fmadd_ps(dy, d_2, fy);
      fz = _mm256_fmadd_ps(dz, d_2, fz);
    }
    ...
    // pvxi = _mm256_fmadd_ps(vdt, _mm256_set1_ps(_hadd_ps(fx)), _mm256_set1_ps(p->vx[i]));
    // pvyi = _mm256_fmadd_ps(vdt, _mm256_set1_ps(_hadd_ps(fy)), _mm256_set1_ps(p->vy[i]));
    // pvzi = _mm256_fmadd_ps(vdt, _mm256_set1_ps(_hadd_ps(fz)), _mm256_set1_ps(p->vz[i]));

    // p->vx[i] = _mm256_cvtss_f32(pvxi);
    // p->vy[i] = _mm256_cvtss_f32(pvyi);
    // p->vz[i] = _mm256_cvtss_f32(pvzi);
  }

  for (u64 i = 0; i < n; i += NB_VALUES_PACKED) {
    //// Positions calculation. 6 Floating operations (12 loop total)
    pxi = _mm256_fmadd_ps(vdt, _mm256_loadu_ps(p->vx + i), _mm256_loadu_ps(p->x + i));
    pyi = _mm256_fmadd_ps(vdt, _mm256_loadu_ps(p->vy + i), _mm256_loadu_ps(p->y + i));
    pzi = _mm256_fmadd_ps(vdt, _mm256_loadu_ps(p->vz + i), _mm256_loadu_ps(p->z + i));
...
int main(int argc, char** argv) {
  ...
  fprintf(stderr, "N-body 3D Problem version 5.0 - using AVX\n\n");
  if (CONFIG) {
